...
local root = repo .. "/nvim/.config/nvim"
...
assert(not treesitter:find("vim.schedule", 1, true), "Treesitter parser installation still runs during startup")
assert(treesitter:find("build = function", 1, true), "Treesitter parser installation is not tied to plugin updates")
...
assert(lsp:find("vtsls =", 1, true), "Vue-compatible TypeScript server is not configured")
assert(lsp:find("globalPlugins", 1, true), "Vue TypeScript plugin is not registered with vtsls")
assert(lsp:find("'vue'", 1, true), "TypeScript server does not attach to Vue buffers")
...
assert(lsp_plugin:find("'vtsls'", 1, true), "vtsls is not managed by Mason")
assert(not lsp_plugin:find("'ts_ls'", 1, true), "ts_ls remains enabled alongside vtsls")
...
assert(git:find("event = 'BufReadPre'", 1, true), "Gitsigns loads too late to attach to the initial buffer")
...
assert(formatting:find("vue = { 'prettierd', lsp_format = 'fallback' }", 1, true), "Vue is not configured to use prettierd")

local autocmds = read("lua/gabre/autocmds.lua")
assert(autocmds:find("'BufEnter'", 1, true), "external file checks do not run when entering a buffer")
assert(autocmds:find("vim.bo.buftype == ''", 1, true), "external file checks do not target normal buffers")
assert(autocmds:find("vim.schedule", 1, true), "external file checks run re-entrantly inside the autocmd")

local tmux_file = assert(io.open(repo .. '/tmux/.config/tmux/tmux.conf', 'r'))
local tmux = tmux_file:read('*a')
tmux_file:close()
assert(tmux:find('set -g focus-events on', 1, true), "tmux does not forward pane focus events")
